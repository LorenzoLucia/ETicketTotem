...
class _ParkingControllerPageState extends State<ParkingControllerPage> {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Text(
              'The ticket for plate "$plate" is valid. You cannot submit a fine.',
            ),
            actions: [
              TextButton(
                onPressed: () {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'Are you sure you want to submit a fine for plate "$plate"?',
          ),
          actions: [
            TextButton(
              onPressed: () {
                ...
                try {
                  ...
                  if (response) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Fine successfully submitted for plate "$plate".',
                        ),
                      ),
                    );
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Failed to submit fine for plate "$plate".',
                        ),
                      ),
                    );
                  }
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Error submitting fine for plate "$plate".',
                      ),
                    ),
                  );
                }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Parking Controller')),
      body: Row(
        children: [
          Expanded(
            flex: 3,
                      ...
                      itemBuilder: (context, index) {
                        return ListTile(
                          ...
                          trailing: IconButton(
                            ...
                            onPressed:
                                () => _removeChalkedCar(_chalkedCars[index]),
                          ),
                        );
                      },
                    ),
